...
class CausalVAE(nn.Module):
    def __init__(self, device="cuda:0", nn='mask', name='vae', z_dim=16, z1_dim=4, z2_dim=4, inference = False, alpha=0.3, beta=1):
        super().__init__()
        self.name = name
        self.z_dim = z_dim
        self.z1_dim = z1_dim
        ...
        nn = getattr(nns, nn)
        self.enc = nn.Encoder(self.z_dim, self.channel).to(device)
        self.dec = nn.Decoder_DAG(self.z_dim,self.z1_dim, self.z2_dim).to(device)
        self.dag = nn.DagLayer(self.z1_dim, self.z1_dim, i = inference).to(device)
        #self.cause = nn.CausalLayer(self.z_dim, self.z1_dim, self.z2_dim)
        self.attn = nn.Attention(self.z1_dim).to(device)
        self.mask_z = nn.MaskLayer(self.z_dim).to(device)
        self.mask_u = nn.MaskLayer(self.z1_dim,z1_dim=1).to(device)

        # Set prior as fixed parameter attached to Module
        self.z_prior_m = torch.nn.Parameter(torch.zeros(1).to(device), requires_grad=False)
        self.z_prior_v = torch.nn.Parameter(torch.ones(1).to(device), requires_grad=False)
    ...
    def negative_elbo_bound(self, x, label, mask = None, sample = False, adj = None, alpha=0.3, beta=1, lambdav=0.001):
        ...
        mask_kl2 = torch.zeros(1).to(self.device)
